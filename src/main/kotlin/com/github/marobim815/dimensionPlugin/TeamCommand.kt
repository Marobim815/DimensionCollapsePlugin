...
class TeamCommand : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        if (args.isEmpty()) {
            player.sendMessage("§c사용법: /팀 [생성/추가/정보] <팀 이름> <플래이어 이름>")
            return true
        }
        ...
        when (args[0].lowercase()) {
            "생성" -> {
                ...
                if (TeamManager.createTeam(teamName, player)) {
                    player.sendMessage("§a팀 '$teamName'이 생성되었습니다!")
                } else {
            ...
            "추가" -> {
                ...
                if (targetPlayer == null) {
                    player.sendMessage("§c플래이어를 찾을 수 없습니다!")
                    return true
                }
                if (TeamManager.addMember(teamName, targetPlayer)) {
                    player.sendMessage("§a${targetPlayer.name}님이 팀'$teamName'에 추가되었습니다!")
                } else {
                    player.sendMessage("§c팀 추가에 실패했습니다!")
                }
            }
            ...
            "정보" -> {
                ...
                if (team == null) {
                    ...
                    player.sendMessage("§6팀 이름: ${team.teamName}")
                    player.sendMessage("§b팀원: ${team.members.joinToString(", ") { it.name }}")
                }
            }
